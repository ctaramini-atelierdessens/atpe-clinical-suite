...
function buildAlerts(session: SessionRow): LocalAlert[] {
  ...
  if ((session.projective_intensity ?? 0) > 80) {
    alerts.push({
      level: 'moderate',
      label: 'Intensité projective élevée',
      detail:
        'L’intensité projective est haute et demande un repérage du niveau de contenance.',
    })
  }

  if ((session.primary_symbolization ?? 0) < 40) {
    alerts.push({
      level: 'moderate',
      label: 'Symbolisation primaire basse',
      detail:
        'La transformation symbolique primaire paraît limitée à ce stade.',
    })
  }

  if ((session.secondary_symbolization ?? 0) < 40) {
    alerts.push({
      level: 'moderate',
      label: 'Symbolisation secondaire basse',
      detail:
        "L’élaboration secondaire semble encore peu disponible ou insuffisamment consolidée.",
    })
  }
...
export function PatientAtpeExpertCard({ sessions }: Props) {
  ...
  const chartData = chronological.map((session, index) => ({
    label: `S${index + 1}`,
    score: clampPercent(getClinicalLevel(session).score),
