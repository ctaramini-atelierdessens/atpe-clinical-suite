...
export default async function PatientGoalsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: goals } = episode
    ? await supabase.from('therapy_goals').select('*').eq('episode_id', episode.id).order('created_at', { ascending: false })
    : { data: [] as any[] }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight">Objectifs thérapeutiques — {patient.code}</h1>
          {episode ? <p className="mt-2 text-sm text-slate-500">Épisode : {episode.episode_label}</p> : null}
        </div>
      ...
      <SectionCard title="Objectifs existants" description="Édition écran par écran via le lien dédié.">
            ...
            <div key={goal.id} className="rounded-2xl border border-slate-200 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">Priorité {goal.priority} · Statut {goal.status}</p>
                </div>
