...
export default async function ExcelImportPage() {
  ...
  const [jobsResult, rowResultsResult, profilesResult] = await Promise.all([
    organization
      ? supabase
          .from('import_jobs')
          .select('*')
    ...
    organization
      ? supabase
          ...
          .select('id, import_job_id, row_number, status, message, patient_id, patient_code')
          .order('created_at', { ascending: false })
          .limit(100)
      : Promise.resolve({ data: [] as any[] }),
    ...
    organization
      ? supabase
          ...
          .select('id, profile_name, profile_scope, config_json, created_at, updated_at')
          .eq('organization_id', organization.id)
  ...
  const rowResults = rowResultsResult.data ?? []
  const mappingProfiles = profilesResult.data ?? []
  ...
  return (
    <div className="space-y-6">
      ...
      <ExcelImportPanel
        jobs={jobs as any[]}
        latestRows={latestRows as any[]}
        mappingProfiles={mappingProfiles as any[]}
      />
    </div>
  )
}
